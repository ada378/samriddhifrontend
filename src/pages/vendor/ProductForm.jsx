...
const emptyProduct = {
  name: '', category: 'iodized-salt', price: '', stock: '', moq: '1',
  packaging: 'Pouch', grade: 'Standard', description: '', images: '',
}
...
export default function VendorProductForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      setLoading(true)
      api.vendor.products({ page: 1, limit: 100 })
        .then(res => {
          const p = res.data.find(x => x.id === id)
          if (!p) { showToast('Product not found', 'error'); return }
          setForm({
            name: p.name || '', category: p.category || 'iodized-salt',
            price: p.price || '', stock: p.stock || '',
            moq: p.moq || '1', packaging: p.packaging || 'Pouch',
            grade: p.grade || 'Standard', description: p.description || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.vendor.updateProduct(id, payload)
    ...
    <div className="admin-page">
      ...
      <form className="admin-form" onSubmit={handleSubmit}>
        ...
        <div className="admin-form-group">
          ...
          <textarea name="images" value={form.images} onChange={handleChange} placeholder="https://example.com/image1.jpg" rows={3} />
        </div>
